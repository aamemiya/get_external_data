...
#    wget --cache=off https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYYMMDDHH}/gfs.t${HH}z.pgrb2.0p50.f${tf}
    ...
    itry=0
    while [ $itry -le 3 ] ; do
      #wget -N --cache=off --timeout=20 https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYY}${MM}${DD}/${HH}/gfs.t${HH}z.pgrb2.0p50.f${tf}
      #wget -N --cache=off --timeout=20 https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYY}${MM}${DD}/${HH}/atmos/gfs.t${HH}z.pgrb2.0p50.f${tf} #### From 2021.3.22 12Z 
      wget -N --cache=off --timeout=20 --no-check-certificate https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYY}${MM}${DD}/${HH}/atmos/gfs.t${HH}z.pgrb2.0p50.f${tf} #### From 2021.9.24 00Z 
      [ $? == 0 ] && break
       itry=`expr $itry + 1`
       echo "retry " $itry " ..."
    done
...
#      echo "$now [CONV] $YYYYMMDDHH -> gfs.$YYYYMMDDHHMMSS_fcst - Plot (foreground job)" >> ${wkdir}/get_ncep_gfs.log
      echo "$now [CONV] $YYYYMMDDHH -> gfs.$YYYYMMDDHHMMSS_fcst - Plot skipped. " >> ${wkdir}/get_ncep_gfs.log
...
#      now=`date -u +'%Y-%m-%d %H:%M:%S'`
#      echo "$now [CONV] $YYYYMMDDHH -> gfs.$YYYYMMDDHHMMSS_fcst - Plot SH (background job)" >> ${wkdir}/get_ncep_gfs.log
...
if ((allget == 1)); then
  now=`date -u +'%Y-%m-%d %H:%M:%S'`
  echo "$now [CONV] $YYYYMMDDHH Plot (background job)" >> ${wkdir}/get_ncep_gfs.log
  cd $wkdir/run/plot_python
  ./plot.sh $YYYYMMDDHH &> plot.log &
  cd -
  now=`date -u +'%Y-%m-%d %H:%M:%S'`
