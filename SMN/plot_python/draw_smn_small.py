...
# print("max min MSLP after smth", np.max(msl), np.min(msl))
 ...
 t2m=shift_array(t2m,smth=4)
 
 uabs10m=np.array(nc.variables['magViento10'][:][0])
 udir10m=np.array(nc.variables['dirViento10'][:][0])

 uabs10m=shift_array(uabs10m,smth=4)
 udir10m=shift_array(udir10m,smth=4)
...
# Lambert Conformal Conic map.
 fig,ax=plt.subplots()
 mp=my_cmap.makemap(ax,lonl,lonr,latl,latr,small=True)
 xl, yl = mp([lonl], [latl])
 xr, yr = mp([lonr], [latr])
...
# ax.clabel(cs,cs.levels,fontsize="small")
 ...
 plt.title('1h precip')
 figname=savedir+"small_prec"+ch+".png"
 fig.savefig(figname)
 plt.close()
...
# add colorbar.
 ...
 ax.text(xr,yr*1.035, 'valid '+vtime.strftime('%d %H:%M:%S'),horizontalalignment='right', verticalalignment='bottom',size="xx-small")
 plt.title('T 2m')
 figname=savedir+"small_t2m"+ch+".png"
 fig.savefig(figname)
 plt.close()
...
# Lambert Conformal Conic map.
 ...
 xe=np.linspace(np.min(x),np.max(x),50)
 ye=np.linspace(np.min(y),np.max(y),50)
...
## draw filled contours.
 ...
 csv = mp.streamplot(xe,ye,u10me,v10me,color="black",linewidth=0.8,density=5.0)
...
# add colorbar.
 ...
 ax.text(xr,yr*1.035, 'valid '+vtime.strftime('%d %H:%M:%S'),horizontalalignment='right', verticalalignment='bottom',size="xx-small")
 plt.title('U,V 10m')
 figname=savedir+"small_uabs10m"+ch+".png"
 fig.savefig(figname)
 plt.close()
