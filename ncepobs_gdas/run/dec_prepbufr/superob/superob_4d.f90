...
!  real(r_sngl) :: refv2d(nlon,nlat,nv2d)
  ...
  real(r_size) :: refpres(nlon,nlat,nlev)

  TYPE(obs_info) :: obsread
  ...
  integer :: nobsgrd(nlon,nlat,0:nlev,nslots,nid_obs)
  integer :: nobsgrdac(0:nlon,nlat,0:nlev,nslots,nid_obs)
  integer :: iobsgrd(nlon,nlat,0:nlev,nslots,nid_obs)
...
!  end do
    ...
    write(*,*) "get_nobs"
    call get_nobs(obsfile, 8, nobs)
  obsread%nobs=nobs

!-------------------------------------------------------------------------------

  allocate ( obsread%lon (nobs) )
  allocate ( obsread%lat (nobs) )
  allocate ( obsread%lev (nobs) )
  allocate ( obsread%dat (nobs) )
  allocate ( obsread%err (nobs) )
  allocate ( obsread%dif (nobs) )
  allocate ( obsread%elm (nobs) )
  allocate ( obsread%typ (nobs) )
...
!===============================================================================
  ...
  call read_obs(obsfile, obsread)

  nobslots=0
  do n = 1, nobs
      tmploti(n) = int( obsread%dif(n) / 3600.0 ) + 4 
      nobslots(tmploti(n)) = nobslots(tmploti(n)) + 1  
      tmpelm(1:n)=real(obsread%elm(1:n))
      tmplon(1:n)=obsread%lon(1:n)
      tmplat(1:n)=obsread%lat(1:n)
      tmplev(1:n)=obsread%lev(1:n)
      tmpdat(1:n)=obsread%dat(1:n)
      tmperr(1:n)=obsread%err(1:n)
      tmptyp(1:n)=real(obsread%typ(1:n))
  end do
...
!    tmploti(nn+1:nn+nobslots(islot)) = islot
    ...
    do n = 1, nobslots(islot)
      ...
      call phys2ijk(refpres, tmpelm(nn+n), tmplon(nn+n), tmplat(nn+n), tmplev(nn+n), &
      ...
      ! surface observation variables (PS/PP/TCobs)
        ...
        if (ceiling(tmprk(nn+n)) > nlev) then
          tmpqc(nn+n) = 3
...
!                    tmp2k   (n3:n4) = tmp2k  (n1:n2)
            ...
            m = nobsgrdac(nlon,nlat,nlev,islot,ielm) - nobsgrdac(0,1,0,islot,ielm)
            if (m > 0) then
              n1 = nnn_before + nobsgrdac(i-1,1,0,islot,ielm) + 1
              n2 = nnn_before + nobsgrdac(nlon,nlat,nlev,islot,ielm)
              n3 = nnn_after + nn + 1
              n4 = nnn_after + nn + m
              tmp2lon (n3:n4) = tmp2lon(n1:n2)
              tmp2lat (n3:n4) = tmp2lat(n1:n2)
...
!    write (supfile(4:5),'(I2.2)') islot
    open (92, file = supfile, form = 'unformatted', access = 'sequential',convert="big_endian")
...
!        write (92) real(elem_uid(tmp2elmi(n)), r_sngl), &
        write (92) real(elem_uid(tmp2elmi(n)), r_sngl), &
                   ...
                   real( (tmp2loti(n) -nbslot) * 3600 , r_sngl)
!      end if
    end do
